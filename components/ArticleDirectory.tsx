...
export default function ArticleDirectory({ posts }: { posts: readonly Post[] }) {
  ...
  return <section aria-labelledby="all-articles-title">
    ...
    <div className="article-categories" aria-label="カテゴリで絞り込む">{categories.map((item) => <button type="button" key={item} className={category === item ? 'active' : ''} aria-pressed={category === item} onClick={() => setCategory(item)}>{item}</button>)}</div>
    ...
    {!filtered.length && <div className="empty-search"><p>一致する記事がありません。別のキーワードをお試しください。</p><button type="button" onClick={() => { setQuery(''); setCategory('すべて'); }}>検索条件をリセット</button></div>}
  </section>;
}
