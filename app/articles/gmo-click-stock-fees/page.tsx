...
export const metadata: Metadata = { alternates: { canonical: '/articles/gmo-click-stock-fees' }, title: 'GMOクリック証券 手数料 高い？｜信用取引無料と別コスト', description: 'GMOクリック証券の手数料が高いと感じる理由を、信用取引の売買手数料0円と買方金利・貸株料、対象外の注文に分けて確認します。' };
...
export default function Page() { const faqJsonLd = { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: FAQS.map((item) => ({ '@type': 'Question', name: item.question, acceptedAnswer: { '@type': 'Answer', text: item.answer } })) }; return <article>
  <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd) }} />
  <p className="page-kicker">GMO CLICK / STOCK COST</p><h1>GMOクリック証券 手数料は高い？<br />信用取引無料と別コスト</h1>
  <p className="lede">GMOクリック証券は通常の現物・信用取引手数料が無料でも、すべての取引や費用が0円とは限りません。高いと感じやすい金利・貸株料などの保有コストと、対象外の注文を分けて確認します。</p>
