...
const products = [
  ...
  { id: 'btc-usd', name: 'ビットコイン BTC/USD', short: 'BTC/USD', quote: 'BTC_MID', unit: 1, unitLabel: '1BTC', marginRate: 0.5, lotStep: 0.01 },
...
export default function FxtfMt5MarginCalculator() {
  ...
  const needsBtcMid = product.quote === 'BTC_MID';
  const needsSecondRate = needsUsdJpy || needsBtcMid;
  const ready = marketPrice !== '' && positive(Number(marketPrice)) > 0 && positive(lots) > 0 && (!needsSecondRate || (usdJpy !== '' && positive(Number(usdJpy)) > 0));
  const yen = new Intl.NumberFormat('ja-JP', { maximumFractionDigits: 0 });

  const result = useMemo(() => {
    const price = positive(Number(marketPrice));
    const quantity = positive(lots) * product.unit;
    const secondRate = positive(Number(usdJpy));
    const referenceRate = needsBtcMid ? (price + secondRate) / 2 : price;
    const notional = referenceRate * (needsUsdJpy ? secondRate : 1) * quantity;
  ...
  }, [marketPrice, lots, product, needsUsdJpy, needsBtcMid, usdJpy, openingFee]);
  ...
  return <div className="calculator-page">
    <header className="calculator-intro">
      ...
      <p className="lede">FXTF MT5の商品CFD4銘柄とBTC/JPY・BTC/USDについて、公式の1Lot単位と個人証拠金率を使って概算します。ライブレートは取得しません。発注時にMT5へ表示された価格を手入力してください。</p>
      <div className="calculator-proof"><span>商品CFDは証拠金率5％</span><span>暗号資産CFDは証拠金率50％</span><span>BTC/USDの公式円換算に対応</span><span>入力内容は保存しません</span></div>
    </header>
      ...
      <div className="calculator-grid">
        <section className="calculator-inputs">
          ...
          <div className="field-grid">
            ...
            <label><span>{needsBtcMid ? 'BTC/JPYの売値（円）' : `${product.name}の価格（${product.quote === 'USD' ? '米ドル' : '円'}）`}</span><input type="number" min="0" step="any" placeholder={needsBtcMid ? 'BTC/JPY Bidを入力' : '取引画面の価格'} value={marketPrice} onChange={e => setMarketPrice(e.target.value === '' ? '' : Number(e.target.value))} /><small>{needsBtcMid ? 'BTC/USDの証拠金円換算に使います' : '売りはBid、買いはAskを入力'}</small></label>
            ...
            <label><span>{needsBtcMid ? 'BTC/JPYの買値（円）' : '米ドル/円の円換算レート'}</span><input type="number" min="0" step="any" placeholder={needsBtcMid ? 'BTC/JPY Askを入力' : needsUsdJpy ? '円換算レートを入力' : 'BTC/JPYでは不要'} value={needsSecondRate ? usdJpy : ''} disabled={!needsSecondRate} onChange={e => setUsdJpy(e.target.value === '' ? '' : Number(e.target.value))} /><small>{needsBtcMid ? '売値・買値の平均を自動計算' : needsUsdJpy ? 'FXTFの計算に使う仲値を確認' : 'BTC/JPYは円建てのため使用しません'}</small></label>
        ...
        <section className="calculator-result">
          ...
          <p>取引数量：{ready ? `${result.quantity.toLocaleString('ja-JP', { maximumFractionDigits: 8 })} ${product.id.startsWith('btc-') ? 'BTC' : product.unitLabel.replace(/^\d+(?:\.\d+)?/, '').trim()}` : '—'}</p>
          <p>証拠金率：{product.marginRate * 100}％（個人口座）</p>
          <p>証拠金＋入力手数料：{ready ? `約${yen.format(result.baseline)}円` : '—'}{openingFee === '' ? '（手数料未入力）' : ''}</p>
          <p>{needsBtcMid ? '計算式：（BTC/JPY売値＋買値）÷2 × Lot数 × 50％' : needsUsdJpy ? '計算式：価格 × 米ドル/円 × 1Lot単位 × Lot数 × 証拠金率' : '計算式：BTC/JPY価格 × Lot数 × 50％'}</p>
        </section>
      </div>
    </div>
    ...
    <nav className="calculator-proof" aria-label="計算根拠と関連記事">
      ...
      <Link href="/articles/fxtf-mt5-btc-jpy-vs-usd">BTC/JPYとBTC/USDの違い</Link>
      <Link href="/articles/fxtf-mt5-silver-cfd-margin">銀1Lotの必要証拠金</Link>
