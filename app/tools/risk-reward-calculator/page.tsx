...
export default function Page() {
  ...
  return <div className="calculator-page">
    <header className="calculator-intro"><p className="page-kicker">RISK / REWARD CALCULATOR</p><h1>マーチンゲール法の計算と<br /><em>損益比・コストを見る。</em></h1><p className="lede">マーチンゲール法の取引額と連敗時の累積額を計算式で確認し、利確幅と損切り幅を円損益へ換算します。往復コストを差し引いたリスクリワード比、損益分岐勝率、1回・複数回の期待値も試算できます。</p><div className="calculator-proof"><span>FX・CFD・株・先物に対応</span><span>往復コストを反映</span><span>損益分岐勝率を算出</span><span>入力は端末内で計算</span></div></header>
